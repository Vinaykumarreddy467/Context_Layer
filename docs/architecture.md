...
`create_handoff` guards against duplicate checkpoints: a record with the same `task_slug` and the same normalized content (`content_hash`, a SHA-256 over whitespace-normalized raw content, migration `0004`) created within 60 seconds is skipped and the existing record is returned, so double-firing lifecycle events (for example PreCompact plus SessionEnd) do not create duplicate history. `platform_session_id` is metadata only and never triggers deduplication. Handoff creation with a `continues_from_id` runs in a SurrealDB transaction, so a failed lineage link leaves no orphan handoff. Capsule export includes the handoff's `version` as `capsule_version`.
